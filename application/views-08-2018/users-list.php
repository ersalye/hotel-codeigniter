                    ...
                    <table class="table table-striped table-bordered file-export">
                        <thead>
                            <tr>
								...
								<th>Past Orders</th>
                                <th>Date of Registered<br> as member </th>
                                <th>Subscribe</th>
                                <th>Total No <br>of Orders</th>
                                <th>Last<br>active date</th>
                               <!-- <th>Rating</th>-->
                        ...
                        <tbody>
								...
								$rid = $p['id']; 
								$CI =& get_instance();
								$aid = $CI->get_user_address($rid);	
								echo $aid['address'];	?></td>
								 <td><a href="<?php echo site_url('Admin/getOrdersbyuser/').$p['id'];?>" class="table-btn btn btn-icon btn-success">Past Orders</a></td>  
                                <td><?php echo $p['created']; ?></td>
                                <td><?php 
								if($p['status'] == 2){
									echo "<sapn class='text-primary'>Active</span>";
								}else{
									echo "<span class='text-danger'>Pending Conformation</span>";
								}

								?></td>
                                <td><?php  $rid = $p['id']; 
								$CI =& get_instance();
								$sid = $CI->Totalordes($rid);	
								echo count($sid);
								?></td>
                                <td><?php echo $p['last_active']; ?></td>

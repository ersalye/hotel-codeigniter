	 ...
	 $('#rg-save').click(function(){
    	...
    	$.ajax({
				...
				$('#valid-otp').show();
					...
					$('#valid-otp').hide();
					$("#sucregister").html('<div style="font-size: 30px;color: green;line-height: 35px;margin-top:5px;text-align:center;"> <div><i class="fa fa-thumbs-o-up" aria-hidden="true"></i></div>Please Confirm Your Email Address to Complete Your Subscription</div>');
				   setTimeout(function() {
					location.reload();
       }, 2000);
				}else{
	 ...
	 $('#OTP-send').click(function(){
    	...
    	$.ajax({
				...
				success: function(data){
					...
					$("#valid-otp").show();	
					$("#lemailf").css("border","1px red solid");;	
					}else{
					$("#sendotp-email").hide();
    	...
    	$.ajax({
				...
				success: function(data){
					if(data.msg=='reset'){
						notif({
				type: "success",
				msg: "<span style='color:#fff'>Password Changed Successfully</span>",
				position: "right",
				width: 500,
				height: 60,
				autohide: true
			});	
			//$("#not-match").html("<span class='btn btn-success'> Password Changed Successfully </span>");

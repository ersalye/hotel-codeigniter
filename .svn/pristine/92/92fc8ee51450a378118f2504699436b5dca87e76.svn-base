 ...
 <div class="app-content content container-fluid">
	...
	if(empty($_GET['start'])){
        ...
        <select class="form-control" id="category" name="rid">
		<option value=''>Select Restaurant</option>
						<?php foreach($shops as $p){ ?>
							<option value="<?php echo $p['id']; ?>"  <?php if (!empty($_GET)){ ?><?=$_GET['rid'] == $p['id'] ? ' selected="selected"' : ''; }?>><?php echo $p['name']; ?></option>
							<?php } ?>
		...
		<div class='col-sm-3'>
            <div class="form-group">
                <div class='input-group date' id='datetimepicker1'>
				  ...
				  <a href="<?php echo base_url('Admin/Paymentsinfo/'); ?>" class="btn btn-danger">Reset</a>
                </div>
            </div>
        </div>
		</form>
                       ...
                       <?php  
							...
							?>
								<tr>
										<td><?php  echo $i; ?></td>
										<td><a href="<?php echo base_url('Admin/Paymentsinfoday/').$dt->format("Y-m-d"); ?>"><?php  echo $dt->format("Y-m-d"); ?></a></td>
										<td><?php echo count($orders_de); ?></td>
										<td><?php echo number_format($totalsale-$tax,2); ?></td>
